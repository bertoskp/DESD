...
public:
   ...
   static std::shared_ptr<BehavioralSpace> load(const std::string &path) {
            ...
            int count=0;
            for (nlohmann::json jTransition : jTransitions) {
                nlohmann::json jState  = jTransition["stato"];
                unsigned int id = jState["id"];
                for (const auto  stato: states){
                    if(stato->getIdd()==id){
                       auto t=transitions.begin()+count;
                        t->get()->setStato(stato);
                        count=count+1;
                        break;
                    }
        ...
        static void save(const std::string &path, BehavioralSpace &bs) {
           ...
           // int numTransitions;
            ...
            for (const auto &transition : bs.transitions) {
                ...
                auto state=transition->getStato();
                jState["id"]=state->id;
