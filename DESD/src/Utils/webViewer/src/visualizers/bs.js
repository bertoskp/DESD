...
export function getBSConfig(j) {
    const jBS = j['behavioralSpace'];

    const transitions = {};
    jBS['transitions'].forEach((t, i) => {
        let label = i + ': ';
        label += t.name;
        if (t.hasOwnProperty('observability')) label += '(' + t['observability'] + ') ';
        transitions[t['id']] = {id: label};
    });

    const states = [];
    jBS['states'].forEach((s, i) => {
        let label = i + ': ';
        label += s['componentsState'].map(cs => cs.state).join(' ');
        label += ' ';

        label += s['linksState'].map(ls => ls.value !== 'empty' ? ls.value : 'ε').join(' ');
        if (s.hasOwnProperty('relevancyLabels')) label += '{' + s['relevancyLabels'].join(', ') + '}';

        if (s.hasOwnProperty('observationState')) label += ' ' + s['observationState'];
        if (s.hasOwnProperty('bondState')) label += ' ' + s['bondState'];
        ...
        states.push({
            ...
            noOutputs: !s.hasOwnProperty('out') || s['out'] === null || s['out'].length === 0
        })
    });
    ...
    return {
        ...
        layout: {
            ...
            klay: {
                ...
                thoroughness: 300 // How much effort should be spent to produce a nice layout..
            },
        }
    }
}