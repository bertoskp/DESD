                                ...
                                do {
                                    ...
                                    switch (choice) {
                                        ...
                                        case 3:
                                            ...
                                            handleDic(maxExecutionTime, inFile, outFile);
...
void read_session( const std::string &path) {
    ...
    string net_name = get_last_chunk(path, "/");
    string net_path = path + "/network_" + net_name + ".json";

    container.networks[net_name] = {};
    container.networks[net_name].net = network_reader(net_path);

    string joined_path = path + "/joined_" + net_name + ".json";
    container.networks[net_name].joined = dictionary_reader(joined_path);

    string dir_path = path + "/MONO";
    read_data(container, net_name, dir_path, MONO);

    dir_path = path + "/OBS";
    if (path_exists(dir_path)) {
        vector<string> dirs = get_directories(dir_path);

        for (auto &d : dirs)
            read_data(container, net_name, d, OBS);
    }
    ...
    if (path_exists(dir_path)) {
        ...
        for (auto& d : dirs)
            read_data(container, net_name, d, SCEN);
    }
}
