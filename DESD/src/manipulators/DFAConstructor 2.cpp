...
//TODO: eliminare le stringhe di log superflue
...
std::shared_ptr<std::vector<std::shared_ptr<DFAState>>> DFAConstructor::subsetConstruction(
    std::shared_ptr<NFASpaceInterface> inputSpace, std::shared_ptr<long> maxExecTime) {
...
std::shared_ptr<Dictionary> DFAConstructor::constructDictionary(std::shared_ptr<NFASpaceInterface> inputSpace,bool online,std::shared_ptr<long> maxExecTime) {

    std::string message;

    Logger::section("Dictionary Construction");

    auto dfaStates = subsetConstruction(inputSpace, maxExecTime);
    auto startingTime = std::chrono::high_resolution_clock::now();
    ...
    for (const auto &state : *dfaStates) {
       ...
       /* pairs=state->getPairs();*/
        ...
        if(!starter && state->nfaStates.find(startingState) != state->nfaStates.end()){
            ...
            auto dicState=std::make_shared<DictionaryState>(isFinal, true, state->in, state->out, state->getDiagnosisOffline(),state->getPairs());
                       dicState->setNfaState(state->nfaStates);
                       std::vector<std::shared_ptr<BehavioralState>> statiNfa;
                       BehavioralState stato;
            ...
            if (online){
                ...
                auto dicState=std::make_shared<DictionaryState>(isFinal, true, state->in, state->out, state->getDiagnosisOffline()
                                                            ,state->getPairs());
                            dicState->setNfaState(state->nfaStates);
                            states.push_back(dicState);
...
void DFAConstructor::epsilonClosure(NFASpaceInterface &space, std::shared_ptr<NFAStateInterface> state,
    ...
    for (const auto &outTransition : state->getOutTransitions()) {
...
std::set<std::set<std::string>> getDiagnosisNewDicState(std::vector<std::shared_ptr<BehavioralState>> totBhs) {
    auto out = std::set<std::set<std::string>>();
    for (const auto nfaState : totBhs) {
